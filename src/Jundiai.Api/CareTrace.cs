...
public static class CareTraceEndpoints
{
    public static IEndpointRouteBuilder MapCareTraceEndpoints(this IEndpointRouteBuilder endpoints)
    {
            ...
            SusBillingEngineStore billing) =>
        {
            ...
            return Results.Ok(CareTraceBuilder.Build(citizen, demo, diagnostics, telemedicine, documents, dental, billing));
        });
            ...
            TelemedicineStore telemedicine) =>
        {
            var citizen = demo.Citizen(citizenId);
            if (citizen is null) return Results.NotFound();
            return Results.Ok(CareTraceBuilder.Continuity(citizen, demo, diagnostics, telemedicine));
        });
        endpoints.MapGet("/api/care-trace/readiness", () => Results.Ok(new
        {
            implemented = new[] { "citizen-root", "regulation", "territory", "ACS", "immunization", "diagnostics", "telemedicine", "documents", "dental", "SUS-production", "pharmacy-dispense", "continuity-gaps" },
...
public static class CareTraceBuilder
{
        ...
        SusBillingEngineStore billing)
    {
        ...
        var continuity = Continuity(citizen, demo, diagnostics, telemedicine);
        return new CareTraceGraph(citizen.Id, citizen.Name, ordered, edges, continuity, DateTimeOffset.UtcNow);
    }

    public static CareContinuitySummary Continuity(Citizen citizen, DemoStore demo, DiagnosticsAdvancedStore diagnostics, TelemedicineStore telemedicine)
    {
